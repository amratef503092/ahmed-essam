...
import 'package:three_m_physics/models/user.dart';
...
class Auth with ChangeNotifier {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (responseData['validity'] == 0) {
          ...
          await SharedPreferenceHelper().setPhoneNumper(_user.phone!);
  ...
  Future<void> logout() async {
    ...
    await prefs.remove(userPref.AuthToken.toString());
    prefs.clear();
  }

  Future<void> updateUserData(User user) async {
    final token = await SharedPreferenceHelper().getAuthToken();
    String url = '$BASE_URL/api/update_userdata';
  ...
  Future<void> updateUserPassword(
    ...
    final token = await SharedPreferenceHelper().getAuthToken();
    String url = '$BASE_URL/api/update_password';
    try {
      final response = await http.post(
        Uri.parse(url),
        body: {
