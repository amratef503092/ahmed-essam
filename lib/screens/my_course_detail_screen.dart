...
import 'package:three_m_physics/models/course_db_model.dart';
import 'package:three_m_physics/models/my_course.dart';
import 'package:three_m_physics/models/section_db_model.dart';
import 'package:three_m_physics/models/video_db_model.dart';
import 'package:three_m_physics/providers/database_helper.dart';
import 'package:three_m_physics/screens/file_data_screen.dart';
import 'package:three_m_physics/widgets/custom_text.dart';
import 'package:three_m_physics/widgets/forum_tab_widget.dart';
import 'package:three_m_physics/widgets/live_class_tab_widget.dart';
...
import 'package:three_m_physics/providers/my_courses.dart';
import 'package:three_m_physics/widgets/app_bar_two.dart';
...
class _MyCourseDetailScreenState extends State<MyCourseDetailScreen>
  ...
  Future<void> processButtonPress(
    ...
    if (mounted) {
      setState(() {
        path = "/data/user/0/com.estaassist.student_app/files/system";
        fileName = lesson.title.toString();
        lessonId = lesson.id;
        courseId = myCourseId;
        sectionId = secId;
  ...
  void didChangeDependencies() {
    {
      ...
      if (_isInit) {
        Provider.of<MyCourses>(context, listen: false)
          ...
          setState(() {
            loadedCoures =
                Provider.of<MyCourses>(context, listen: false).myCourse;
          });
          if (val != null) {
            setState(() {
              loadedCoures = val;
  ...
  void lessonAction(Lesson lesson) async {
    // debugPrint("lessonType is >> ${lesson.lessonType}");
    // debugPrint("vidUrl is >> ${lesson.videoUrl}");
    // debugPrint("vidUrl_Web is >> ${lesson.videoUrlWeb}");
    // debugPrint("videoTypeWeb is >> ${lesson.videoTypeWeb}");
    // debugPrint("attachmentType is >> ${lesson.attachmentType}");

    if (lesson.lessonType == 'video') {
      if (lesson.videoTypeWeb == 'html5' ||
              lesson.videoTypeWeb ==
                  'amazon' /*  ||
          lesson.videoTypeWeb == "omega_stream" */
          ) {
        Navigator.push(
          context,
          MaterialPageRoute(
        ...
        final token = await SharedPreferenceHelper().getAuthToken();
        var url =
            '$BASE_URL/api_files/file_content?course_id=${(widget.courseId).toString()}&lesson_id=${lesson.id}&auth_token=$token';
        // print(url);
        Navigator.push(
          context,
          MaterialPageRoute(
  ...
  Widget build(BuildContext context) {
    ...
    final sections =
        Provider.of<MyCourses>(context, listen: false).sectionItems;
    myCourseBody() {
      return _isLoading || /* loadedCoures == null ||  */ myLoadedCourse == null
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      onChanged:
                                                                        ...
                                                                        Provider.of<MyCourses>(context,
                                                                                listen: false)
                                                                            .toggleLessonCompleted(int.parse(lesson.id), value! ? 1 : 0)
                                                                    ...
                                                                    : Expanded(
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          onChanged:
                                                                            ...
                                                                            setState(
                                                                              () {
                                                                                lesson.copyWith(isCompleted: value! ? '1' : '0');
                                                                                if (value) {
                                                                            ...
                                                                            Provider.of<MyCourses>(context, listen: false).toggleLessonCompleted(int.parse(lesson.id), value! ? 1 : 0).then(
                                                                                  (_) => CommonFunctions.showSuccessToast('Course Progress Updated'),
                                                                                );
                                                                          },
                                                                        ),
                                                                      )
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      onChanged:
                                                                        ...
                                                                        Provider.of<
                                                                          ...
                                                                          listen:
                                                                              false,
                                                                        ).toggleLessonCompleted(int.parse(lesson.id), value! ? 1 : 0).then((_) =>
